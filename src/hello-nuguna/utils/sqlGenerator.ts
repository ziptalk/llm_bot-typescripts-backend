...
const translateToEnglish = async (text: string): Promise<string> => {
  ...
  try {
    ...
    logger.warn('Primary translation model failed. Attempting with fallback models.');
    for (const fallbackEndpoint of translationFallbackEndpoints) {
      try {
        return await callTranslationModel(fallbackEndpoint, text);
      } catch (fallbackError) {
        logger.error(`Translation failed with model at ${fallbackEndpoint}`, {
          error: fallbackError instanceof Error ? fallbackError.message : fallbackError,
        });
      }
    }
    logger.error('Translation failed using all models.');
...
const callTranslationModel = async (endpoint: string, text: string): Promise<string> => {
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      if (status === 503) {
        logger.error('Translation API service temporarily unavailable (503)', { endpoint });
      } else if (status === 404) {
        logger.error('Translation API endpoint not found (404)', { endpoint });
      } else {
        logger.error('Translation API call failed', { endpoint, error: error.message });
      }
    } else {
...
export const generateSQLQuery = async (question: string): Promise<string> => {
  ...
  if (isLocal && !HUGGING_FACE_API_KEY) {
    ...
    logger.warn('Local environment without API Key detected. Using default SQL query.');
  ...
  while (attempt < MAX_RETRIES) {
    ...
    if (isLocal && sqlQuery === 'function') {
      logger.warn(`Received "function" as query in local environment. Using default query instead.`);
      return generateDefaultSQLQuery(question);
    }
    ...
    logger.error('Text-to-SQL API call failed', { error: error instanceof Error ? error.message : error });
    break;
  }
}
}
  ...
  logger.warn('All attempts to generate SQL query failed. Using default query.');
  return generateDefaultSQLQuery(question);
};
